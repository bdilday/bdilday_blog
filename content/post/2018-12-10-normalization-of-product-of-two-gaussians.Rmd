...
\phi(\mu1 | \mu_2, \sigma_{12}) \phi(x | mu, \sigma)$$, 
...
$$\mu = \frac{\mu_1/\sigma_1^2 + \mu_2^2/\sigma_2^2}{1/\sigma_1^2 + 1/\sigma_2^2}$$
...
If I look at the arguments of the exponentials, supressing the $-\frac{1}{2}$ factor, I can expand out the terms to get
...
Now, generically, if I have an expression $A x^2 + B x + C$, I can complete the square by adding and subtracting $\frac{B^2}{4 A} - \frac{C}{A}$. To see this, note that if we have a perfect square of $(x-k)$ for some $k$, then, 

$$ (x-k)^2 = x^2 - 2 x k + k^2$$ 

and write

$$ A x^2 + B x + C = A (x^2 + \frac{B}{A} + {C}{A})$$
...
$$A x^2 - \frac{B} x + \frac{C} = A (x - k)^2 - A \delta$$.
...
So in $B^2/4 A - C$ the terms $\mu_1 \sigma_2^4$ and $\mu_2 \sigma_1^4$ cancel out and the t erm $\sigma_1^2 \sigma_2^2$ divides out, leaving,

$$ \frac{1}{\sigma_1^2 + \sigma_2^2}(\mu_1^2 + \mu_2^2 - 2 \mu_1 \mu_2) \\
= \frac{1}{\sigma_1^2 + \sigma_2^2}(mu_1 - mu_2)^2$$
